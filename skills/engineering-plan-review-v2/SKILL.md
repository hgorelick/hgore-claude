...
**The spec binds through the brief, and only through it.** Grounding at this layer is parental: the plan is prosecuted against its brief and the repo, and this skill never reads `vision.md`. The parent spec reaches the review through one channel — the brief's `**Spec:**` header, resolved by file presence — and constrains three things: which surface this plan's chunks may implement (the `## Decomposition` Coverage table), where a boundary-adjacent chunk belongs (a seam's split-line predicate), and which boundary calls are already settled (Active `Status: bound` entries in the spec's decisions logs — the per-spec log and the shared `specs/decisions.md`, nearest first — which bind **transitively** here and are never re-litigated at this layer). No header resolving to a real spec, or a spec with no `## Decomposition`, and every spec-layer check below is a recorded no-op: the plan is prosecuted against its brief and the repo alone, and no other stage changes.

**Sibling tracks are one feature.** When this feature has more than one engineering plan, the plan under review is one track of a co-delivered whole (`~/.claude/skills/_plan-common/layout.md`) — nothing goes live on a merge, so the feature ships only through a deliberate whole-feature deploy. Do not review the track as independently shippable. A track delivering no brief Goal on its own, a chunk consumed only by a sibling track, and cross-track wiring left to the sibling's own cycle are the feature's structure, not orphan / integration / go-live / undelivered-Goal defects — file none of them (`~/.claude/skills/_review-common/principles.md` § Sibling-plan co-delivery, `_review-common/critical-pairs.md` § P-EP-SIBLING-CODELIVERY). Coverage still binds: a Goal **no** sibling track delivers is a real gap, and a cross-track shared contract whose export and import have drifted is a real finding. A sibling carrying the `/ep-close` closure marker is a sealed contract: never file a finding whose resolution adds a chunk to it or amends it — resolution paths route the scope into the plan under review, a new track, or a new feature (`_plan-common/layout.md` § Closed engineering plans).
...
Ground Truth Pass               (deterministic, no LLM judgment)
  ↓ resolves the parent spec from the brief's `**Spec:**` header by file presence
  ↓   (no header → spec-layer checks record N/A; header naming a missing file →
  ↓    N/A plus OPEN_QUESTION; no `## Decomposition` → `Decomposition trace:
  ↓    N/A — no decomposition resolved`; nothing else changes in any of them)
  ↓ runs the Decomposition trace at chunk grain — no chunk implements surface the
  ↓   Coverage table gives a sibling brief; a boundary-adjacent chunk is placed by
  ↓   the seam's split-line predicate
  ↓ produces audit_report (incl. decision-closure audit with
  ↓ prior-classification consistency check against the decisions logs —
  ↓ the feature's own, and the parent spec's Active bound entries in
  ↓ `specs/<slug>/decisions.md` + `specs/decisions.md`, nearest first)
...
Orchestrator Decision           (deterministic + judgment)
  ...
  ↓ runs carry-forward consultation Priority 1 (both decisions logs — boundary
  ↓   findings matched against the parent spec's) then
...
`Status:` is a ternary signal: `needs-user-input` (mid-cycle), `closed` (implementation complete, written only by `/ep-close`), or absent (ready). Other lifecycle signals (Frozen, Archived) come from git/PR state, not frontmatter.
...
- **`Status: closed`** → stop. Do NOT spawn anything. The plan is implementation complete and sealed by `/ep-close` (`~/.claude/skills/_plan-common/layout.md` § Closed engineering plans) — there is nothing left to review, and a review round would manufacture amendments a closed plan cannot accept. Emit:

  ```
  STATUS: REFUSED (engineering plan closed — implementation complete)

  This engineering plan carries the `/ep-close` closure marker. A closed plan
  accepts no new chunks and no amendments. New scope routes to an open sibling
  track, a new track, or a new feature (layout.md § Closed engineering plans).
  Reopening is a director-only act: remove the marker and supersede the closure
  entry in decisions.md, then re-invoke.
  ```

- **No `Status:` field, OR any other value** → proceed normally. The Round Memory Pass consults the engineering-plan-author sidecar at `~/.claude/cache/author-state/<feature>[__<track>]__engineering-plan.json` — author and reviewer address one plan by one slug, derived per `~/.claude/skills/_plan-common/layout.md` § State-slug derivation; if `authoring_mode: "draft"` is set there (the plan was written via `/engineering-plan-author --draft`, skipping Plan-lint, Concern-lint, Ground-truth, Self-prosecution, and Imagined-Implementer), the verdict surfaces a draft warning. Persona prosecution still runs.
...
**Legacy-slug migration.** A bare `<feature>.json`, with no layer suffix, is state stranded under a name no author skill reads: `/brief-author`, `/engineering-plan-author`, and `/plan-author` all consult the canonical slug above for warm carry-forward, so anything left at the bare name is a dead channel. On load: read the canonical slug first; if absent, fall back to `<feature>.json` and treat it as this plan's prior state. On persist: always write the canonical slug, and delete the legacy file once the canonical one is written. Report the migration on the verdict's `State source` line.
...
**Blocker classes seen here** — `BRIEF_AMENDMENT_NEEDED`, `SPEC_AMENDMENT_NEEDED`, `IMPLEMENTABILITY_GAP`, `UNCORROBORATED_RESET`, `CHUNK_SURFACE_EXCESS`, `FEATURE_SURFACE_EXCESS`, `FEATURE_NONCONVERGENCE`, `AUTHOR_GATE_DRIFT`, plus the universal three.
...
Sub-passes in order: Parent-spec resolution, Brief Trace, Decomposition trace, Repo Reality, Structural Lint (style), Chunk-surface estimator, Decision-Closure Audit, Mechanical Fixes. Persona Prosecution agents MUST NOT re-prosecute facts verified here.

### Parent-spec resolution (mechanical; degrades by file presence)

`Read` the brief's `**Spec:**` header. It names the spec this feature's brief descends from, and the header is the only channel by which the spec reaches this layer — grounding is parental, so the review never resolves a spec the brief did not claim, never asks which one, and never reads `vision.md`.

- **Header present and the path resolves to a file** → that is the parent spec. Read its `## Decomposition` (Seams, Briefs table, this slug's scope stub, Coverage table) and its decisions logs — `specs/<slug>/decisions.md` beside a per-system spec **and** the shared `specs/decisions.md` alongside it, nearest first (the per-spec log wins where both bind the same call); root `decisions.md` beside a root spec that carries `## Decomposition`. Record `parent_spec: <path> (from the brief's Spec: header)`.
- **Header present but the path resolves to nothing** → `[HARD: brief's Spec header names a file that does not exist]`, filed against the brief as `OPEN_QUESTION` (HIGH), record `parent_spec: named but missing — <path>`, and the spec-layer checks record `N/A`. It is a brief claim about a real file, and every trace below would anchor on it. It is not the no-header case: that one is silent, this one is a finding.
- **No `**Spec:**` header** → record `parent_spec: none (brief carries no Spec header — legacy layout)`. The Decomposition trace records `N/A`, the spec logs are not consulted, and the plan is prosecuted against its brief and the repo alone — no other stage changes. Not a finding: whether the brief carries a header is the brief layer's business, prosecuted there.
- **Parent spec resolves but carries no `## Decomposition`** → record `Decomposition trace: N/A — no decomposition resolved` (the reviewer-side name for the condition the author sidecar records as `decomposition_stub: absent`). Boundary-binding is gated on the decomposition: a spec that draws no boundaries has none to bind, so its logs are not read for boundary arbitration either, and the Spec-log consultation is a recorded no-op.
...
### Decomposition trace (mechanical; runs only when the parent spec carries `## Decomposition`)

The Brief Trace asks whether every chunk traces to the brief. This asks the sharper question the Coverage table can answer: whether the surface those chunks implement is *this brief's* to implement. A spec with no `## Decomposition` — or no parent spec at all — records `Decomposition trace: N/A — no decomposition resolved` and nothing else in the Ground Truth Pass changes.

The slug the table is read against is the brief's own: the feature directory name under `features/` **is** the slug, and it matches the brief's row in the spec's Briefs table (`~/.claude/skills/_spec-common/spec-format.md` § Scope stubs). A Briefs-table row matching no feature directory, or a feature directory matching no row, is the mismatch to flag. A Coverage table with **no row for this slug** short-circuits the trace rather than failing every chunk in it: record `decomposition_trace: no row for <slug>`, file exactly one `OPEN_QUESTION` naming the slug and the missing row, and let the no-annexation check below stand down for the round. One missing row is one finding — never one per chunk, and never a silent proceed.

Read the section's **Seams** block, this slug's **scope stub**, and the **Coverage** table in full. Then, for each chunk-index row, identify the spec units its stated concern implements — the invariants, feature areas, and terms its work ranges over. Two checks, both substring-and-enumeration work against authored lists, neither a judgment call:

- **No annexation.** Every unit a chunk implements must be one the Coverage table dispositions to **this brief's slug**. A chunk implementing a unit the table assigns to a **sibling brief**, or excludes by a **named seam**, is `[HARD: annexes <slug>'s surface]` → escalate as `OPEN_QUESTION` naming the unit, the disposition the table gives it, and the chunk that reached for it. Do not widen the plan and do not drop the chunk: which of the two is right is the director's call. This is the finding no other stage can make — the annexing chunk traces to a real brief Goal and passes the Brief Trace clean, the annexed brief still traces its own, and both artifacts review clean while the unit ships twice or nowhere.
- **Seam placement.** A chunk whose concern sits near a named seam is placed by that seam's split-line predicate: hand the predicate the chunk's concern and take the side it returns, with no further argument. A chunk the predicate returns to the far side is the same finding, filed with the seam name and its predicate quoted verbatim. A seam carrying no usable predicate is the spec layer's defect, not this plan's — record it and route it as `SPEC_AMENDMENT_NEEDED` rather than guessing the placement.

The trace runs forward only. The inverse direction — does every outcome the stub owes get delivered — is the brief layer's check, already run by `/brief-review-v2`'s Decomposition trace and re-expressed here as the Brief Trace's Goal coverage.

```
### Decomposition Trace
Parent spec: <path>   Slug claimed: `<brief-slug>`
Coverage rows for this slug: <count>   Seams: <count>
Chunks → implemented units:
- `<chunk-slug>` → implements "<unit>" → Coverage: `<this-slug>` ✓
- `<chunk-slug>` → implements "<unit>" → Coverage: `<sibling-slug>`  [HARD: annexes <sibling-slug>'s surface]
- `<chunk-slug>` → implements "<unit>" → Coverage: excluded by <seam name>  [HARD: annexes excluded surface]
- `<chunk-slug>` → sits at <seam name>; predicate "<verbatim>" returns <side>  ✓ / [HARD: chunk placed across the seam]
```
...
- **Legacy tolerance.** A plan with no Invariants section, no Threat model section, or invariants carrying neither field is in the legacy shape. File ONE `[SOFT LOW]` per missing convention recommending migration on the next substantive edit; do not file per-entry findings and do not treat it as a structural defect.

**Chunk index column rules:**
- Columns are EXACTLY `Slug | Chunk | Intent | Code deps`. Any of `#`, `Status`, `PR`, `Mode`, `Owner`, `Last-updated` is `[HARD]`. A missing `Intent` column is `[SOFT LOW]` (migrate on next touch), not `[HARD]` — the legacy shape, recommended for migration rather than refused.
...
**Spec-log consultation (MANDATORY when a parent spec resolved).**

`Read` the parent spec's decisions logs alongside the feature's own — `specs/<slug>/decisions.md` beside a per-system spec **and** the shared `specs/decisions.md` alongside it, nearest first (the per-spec log wins where both bind the same call); root `decisions.md` beside a root spec that carries `## Decomposition`. Only `## Active (bound)` entries bind in any of them; a `superseded` / `obsolete` entry in the `## Archived` tail never arbitrates (per `~/.claude/skills/_review-common/principles.md` § What counts as a bound entry). The logs **split by subject**: a call about which briefs exist or where a boundary sits lives in the spec's log; a call inside one feature's scope lives in the feature's log. Bound decisions are transitive — a boundary or seam call bound one layer up binds this plan and is **never re-litigated here**.

- **Plan honors the bound boundary** → record as `verified: bound_in_spec_log`.
- **A chunk's placement contradicts a bound boundary or seam call** → `[HARD: chunk placement contradicts a bound boundary]`, escalated as `OPEN_QUESTION` for `/spec-author`, quoting the entry verbatim alongside the chunk row. The orchestrator never re-cuts a boundary from inside a plan review and never writes the spec's log — re-cutting is `/spec-author`'s Seam alignment, and it supersedes the bound entry there the log's usual two-step way.
- **A fix applied this round would move a bound boundary** → `FIX_INTRODUCED_PREMISE_INVERSION`, unless the director explicitly re-cut upstream first. Applying it anyway silently overrides an arbitration made one layer up, where it also binds the sibling brief on the other side.

No parent spec resolved, no log beside it, or a parent spec carrying no `## Decomposition`, and this consultation is a recorded no-op — a spec that draws no boundaries has none to bind, so nothing beside it is read as a parent-spec log.
...
Spec logs: {<per-spec path> + {specs/decisions.md} | none — no parent spec resolved | none — spec carries no decomposition}; bound boundary entries consulted: <count>; placement contradictions: <count>
...
- parent_spec: `<path> (from the brief's Spec: header)` | `none (brief carries no Spec header)` | `named but missing — <path>`
...
- decomposition_trace: `N/A — no decomposition resolved` | `no row for <slug>` | chunks_claiming_sibling_surface, chunks_placed_across_a_seam, coverage_rows_for_slug, seams
...
- decision_closure: deferrals_total, cross_chunk_wiring, brief_layer, chunk_internal, unclear, hard_findings, spec_log_entries_consulted, placement_contradictions
...
- Editing the brief just to make a chunk fit → `BRIEF_AMENDMENT_NEEDED`, **after the routing step below**.
- Editing the parent spec, its `## Decomposition`, or its decisions logs → never, under any class. Those are `/spec-author`'s, and a boundary re-cut from inside one feature's review binds nothing for the sibling brief on the other side of it.
- "Will be cleaned up later" — if it's not in the plan now, it won't happen.

**Amendment routing (runs before any `BRIEF_AMENDMENT_NEEDED` is filed).** Ask where the contradiction actually originates. A defect genuinely the brief's own — a Goal it under-specifies, a Non-goal it should carry, an environmental claim it invented — keeps `BRIEF_AMENDMENT_NEEDED`. A contradiction originating **upstream of the brief** — in the parent spec's `## Decomposition` (a seam, a Coverage assignment, a scope stub) or in the spec's prose — files `SPEC_AMENDMENT_NEEDED` routed to `/spec-author` instead, naming the spec section that owes the change. Amending the brief in that case papers over the layer that actually owes it, and the next brief cut from the same spec inherits the same contradiction. Route **one layer at a time**: never escalate past the spec, because a contradiction rooted in a vision-layer rule reaches `vision.md` through the spec machinery (`/spec-author` files `VISION_AMENDMENT_NEEDED`), never from this review. With no parent spec resolved, routing is a no-op and every amendment is the brief's.
...
**Priority 1 — Decisions logs (durable record), class-aware.** `Read` `features/<feature>/decisions.md` if it exists, **and the parent spec's decisions logs alongside it** — `specs/<slug>/decisions.md` and the shared `specs/decisions.md`, nearest first (Ground Truth's Spec-log consultation already loaded them). The retraction logic is split by finding class per `_review-common/principles.md` § Cross-artifact authority order, and findings split across the logs by subject the same way the logs themselves do: a finding about cross-chunk wiring inside this feature is matched against the feature's log; a **boundary finding** — which brief owns a unit, where a seam falls, which side of one a chunk belongs on — is matched against the spec's, because that is where a boundary call is bound. A finding that would re-cut a boundary an Active bound entry there fixes is dropped exactly as a feature-scoped finding is dropped against the feature log; where it survives as substance, it escalates as `OPEN_QUESTION` for `/spec-author` rather than being applied here. This skill never writes the spec's logs.
...
**For Class B and C findings only**, scan the applicable log — the feature's `decisions.md`, or the parent spec's for a boundary finding — for entries where ALL of:
...
**Personas:** {names}
**Parent spec:** {resolved path (from the brief's `Spec:` header) | `none — brief carries no Spec header` | `named but missing — <path>`} {; decomposition: present | absent}
**Ground Truth audit:** brief_trace PASS / N hard findings; decomposition_trace PASS / N hard / N/A; repo_reality PASS / N hard findings; structural_lint PASS / N findings; decision_closure: {n cross-chunk-wiring HARD, n brief-layer HARD, n boundary-placement HARD, n reclassification → OPEN_QUESTION, n unclear → Persona Prosecution}
...
**Carry-forward consultation:**
  - Priority 1 (decisions logs): findings checked: {n}; retracted via bound feature entry: {n}; retracted via bound spec-log boundary entry: {n} (spec logs: {per-spec path + specs/decisions.md | none})
...
### Decisions to bind (yours) — only under APPROVED
...
- [BRIEF_AMENDMENT_NEEDED] {finding} — {what brief change is needed and why}; routing checked: the contradiction originates in the brief itself, not upstream of it.
- [SPEC_AMENDMENT_NEEDED] {finding} — the contradiction originates upstream of the brief, in {the spec's `## Decomposition` (seam / Coverage assignment / scope stub) | the spec's prose at §<heading>}: "{verbatim}". Routed to `/spec-author`; amending the brief would leave the next brief cut from this spec inheriting it.
- [STABLE_DISAGREEMENT] {finding} — Persona A proposes {fix A}; Persona B proposes {fix B}. Pick one.
- [OPEN_QUESTION] {finding} — {question}
- [IMPLEMENTABILITY_GAP] {finding} — cross-chunk decision/identifier the engineering plan must bind. Source: {Decision-Closure Audit | Imagined-Implementer}. (Only under NEEDS USER INPUT — under APPROVED these render in the Decisions-to-bind section above, never here.)
...
**Rendering rule for APPROVED.** An outstanding `IMPLEMENTABILITY_GAP` under APPROVED is a decision, not a defect — render it only in `### Decisions to bind (yours)` and leave `### Blockers` empty (omit the section). APPROVED with a non-empty Blockers section is a rendering bug: whatever would go there means the verdict is NEEDS USER INPUT. Under NEEDS USER INPUT everything, implementability gaps included, renders in Blockers as one list.
...
- **Round Memory pass is mandatory.** Skipping it disables the plan-growth gate and section-diff gate, and the review thrashes — each round re-prosecutes the remediation the last round produced. State file lives at `~/.claude/cache/review-state/<feature>[__<track>]__engineering-plan.json` (NOT in the project repo).
...
- **The Decomposition trace is mandatory whenever the parent spec carries `## Decomposition`.** No chunk implements surface the Coverage table gives a sibling brief or excludes by a named seam, and a boundary-adjacent chunk is placed by that seam's split-line predicate. It is the one check that catches a plan quietly annexing a neighbouring brief's scope, because annexation passes every other stage — the chunk traces to a real brief Goal, the annexed brief still traces its own, and both artifacts review clean. No parent spec, or a spec with no decomposition, records `Decomposition trace: N/A — no decomposition resolved`; that skip is the legacy path, not a shortcut.
- **The parent spec's decisions logs are consulted alongside the feature's own** — `specs/<slug>/decisions.md` and the shared `specs/decisions.md`, nearest first — at the Decision-Closure Audit's Spec-log consultation and again at Priority-1 carry-forward. Bound decisions are transitive: an Active `Status: bound` boundary or seam call there binds this plan and is never re-litigated at this layer. A chunk placement contradicting one escalates as `OPEN_QUESTION` for `/spec-author`; a fix that would move a bound boundary is `FIX_INTRODUCED_PREMISE_INVERSION` unless the director re-cut upstream first. Boundary-binding is gated on the parent spec carrying `## Decomposition` — a spec that draws no boundaries has none to bind, and nothing beside it is a parent-spec log here. This skill never writes those logs.
- **Amendments route one layer at a time.** Before filing `BRIEF_AMENDMENT_NEEDED`, ask whether the contradiction originates upstream of the brief — in the spec's `## Decomposition` or its prose — and file `SPEC_AMENDMENT_NEEDED` routed to `/spec-author` when it does. The brief class is reserved for defects genuinely the brief's own. Never escalate past the spec: a vision-layer contradiction reaches `vision.md` through the spec machinery, not from this review.
...
- **Never** mark CLOSED while any blocker class (`BRIEF_NONGOAL_TRESPASS`, `BRIEF_GOAL_UNDELIVERED`, `SURFACE_PARITY_GAP`, `GOAL_VERIFICATION_GAP`, `BRIEF_AMENDMENT_NEEDED`, `SPEC_AMENDMENT_NEEDED`, `STRUCTURAL_LINT_FAILED`, `CHUNK_SURFACE_EXCESS`, `FEATURE_SURFACE_EXCESS`, `FEATURE_NONCONVERGENCE`, `AUTHOR_GATE_DRIFT`, `STABLE_DISAGREEMENT`, `OPEN_QUESTION`, `IMPLEMENTABILITY_GAP`, `UNCORROBORATED_RESET`, `FIX_INTRODUCED_PREMISE_INVERSION`, `REPO_PREMISE_GAP`, `REPO_STATE_DRIFT`) is non-empty. This is the registry's Engineering-plan-review gate (`~/.claude/skills/_review-common/blocker-classes.md` § Verdict gates) stated locally; the two lists are the same list.
...
- **Never** edit the brief just to make a chunk fit. That's `BRIEF_AMENDMENT_NEEDED` — or `SPEC_AMENDMENT_NEEDED` when the routing step finds the contradiction originates upstream of the brief. Never edit the parent spec, its `## Decomposition`, or its decisions logs in either case.
...
- **Brief and plan disagree on a Goal** → run the amendment routing first. The brief is canonical, so a disagreement the brief itself owns is `BRIEF_AMENDMENT_NEEDED` and the user signs off on the amendment or the plan changes. A disagreement whose root is upstream — the Goal traces to a spec unit the Coverage table gives a sibling brief, or the spec's prose contradicts what the Goal commits — is `SPEC_AMENDMENT_NEEDED` routed to `/spec-author`.
- **Brief carries no `**Spec:**` header** → no parent spec resolves. Record `parent_spec: none`, skip the Decomposition trace and the Spec-log consultation, and prosecute the plan against its brief and the repo alone — no other stage changes. Not a finding here: the header is the brief's claim and `/brief-review-v2`'s Stage 0 decides whether one is owed.
- **`**Spec:**` header names a file that is not on disk** → `[HARD]` against the brief, filed as `OPEN_QUESTION` (HIGH), `parent_spec: named but missing — <path>`, spec-layer checks record `N/A`, and the review proceeds. Every spec-layer trace anchors on that path, so a header pointing at nothing is the one brief claim this review cannot recover from — and it is a finding where a missing header is silent.
- **Parent spec resolves but carries no `## Decomposition`** → `Decomposition trace: N/A — no decomposition resolved`, the same condition the author sidecar records as `decomposition_stub: absent`. Boundary-binding is gated on the decomposition, so the Spec-log consultation is a recorded no-op too: a spec that draws no boundaries has none to bind.
- **Parent spec's Coverage table has no row for this brief's slug** → do not fall back to tracing chunks against the whole spec. The slug is the feature directory name under `features/`, matching the brief's row in the spec's Briefs table, so a Briefs-table row matching no feature directory (or a feature directory matching no row) is the mismatch to name. Record `decomposition_trace: no row for <slug>`, short-circuit the trace, and file exactly one `OPEN_QUESTION` naming the missing row — never one per chunk. Either the brief names the wrong parent or the spec's decomposition never cut this brief, and both are answered upstream.
- **A chunk implements a spec unit the Coverage table gives a sibling brief** → `OPEN_QUESTION`, never an auto-fix in either direction. Do not widen the plan to keep the chunk and do not drop it; the director picks, and the pick lands as a bound entry in the spec's log via `/spec-author`.
...
- **State file missing AND no `decisions.md`** → cold start. Round 1, empty `prior_blockers`, empty `recently_resolved_blockers`. No reconstruction; full re-prosecution latitude.
...
- **`/spec-author` and `/spec-review`** own the parent spec, its `## Decomposition`, and its decisions logs — the slice this feature's brief was cut from, the Coverage table the Decomposition trace prosecutes chunks against, and the seams whose split-line predicates place a boundary-adjacent chunk. Their Active `Status: bound` boundary calls **bind transitively here**: they are constraints this review honors and never re-litigates. A boundary this plan cannot honor, and any contradiction originating in the spec rather than the brief, goes back to that pair as `OPEN_QUESTION` / `SPEC_AMENDMENT_NEEDED`. This skill never edits the spec or its logs, and never routes past the spec to `vision.md` — grounding is parental, so this review reads its brief and the repo, and vision reaches it only through the spec map and the spec.
- **`/brief-author` and `/brief-review-v2`** own the brief this plan is grounded in, and run the same three checks one layer up: parent-spec resolution by file presence, the no-annexation check against the Coverage table, and the split of the decisions logs by subject. A defect genuinely the brief's own comes back to them as `BRIEF_AMENDMENT_NEEDED`; the routing step is what keeps a spec-layer defect from arriving there wearing a brief-layer label.
- **`/engineering-plan-author`** writes the plan this skill prosecutes and runs the same spec-layer reads at write time (Source ingest § Parent-spec resolution, the chunk-grain no-annexation check, the transitive-bound-decisions rule). Its sidecar's gate fields are cross-checked here; disagreement files `AUTHOR_GATE_DRIFT`.
- **`/plan-review-v2`** reviews the chunk plans this layer's CLOSED verdict unblocks. A boundary bound upstream reaches it through this plan's chunk placement, not by re-reading the spec.
