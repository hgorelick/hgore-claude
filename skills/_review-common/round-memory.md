...
Loaded by `/brief-review-v2`, `/plan-review-v2`, `/engineering-plan-review-v2`, `/review-pr-v2`, `/spec-review`, `/vision-review`. This file defines the mechanism: where state lives, what it holds, how it loads, and how it persists. The hosting skill names its slug shape, its extra schema fields, and any extra sub-passes it runs on top.
...
For artifacts under `features/`, slug derivation follows `_plan-common/layout.md` § State-slug derivation — the path relative to `features/`, minus the `plans/` and `implementation/` segments, `/` replaced by `__`. The PR, spec, and vision layers are keyed differently, since none of those artifacts lives under `features/`:
...
| Spec | `<project>__<spec-slug>__spec` where `vision.md` exists at the repo root and specs live under `specs/<slug>/`; `<project>__spec` in a project whose root artifact is a single `spec.md`. `<project>` is the repo-root basename |
| Vision | `<project>__vision` |
...
**Legacy field names — read both, write canonical.** State files on disk carry either name. On read, accept both; on write, emit only the canonical one and drop the legacy key.
...
**Legacy slugs.** Where a host's state file on disk carries a different slug, the host reads the canonical slug first, falls back to that name, and on persist writes canonical and deletes the other file. Report the migration on the verdict's `State source` line.
...
3. A decisions-log entry added since `last_review_at` whose subject matches the blocker's `path_or_section` — use its `Why:` paragraph. Which logs are read is layer-specific, nearest first: `features/<feature>/decisions.md` at the brief and chunk-plan layers; that log **and** the parent spec's (`specs/<slug>/decisions.md`, then `specs/decisions.md`) at the engineering-plan layer; the spec's own log and `specs/decisions.md` at the spec layer; `specs/decisions.md` at the vision layer
