...
Loaded by `/review-pr-v2`, `/plan-review-v2`, `/engineering-plan-review-v2`, `/brief-review-v2`, `/spec-review`. The hosting skill defines workflow; this file defines stance.
...
**Class A — Brief contract.** The brief is the authoring user's product contract: what is being built, what is explicitly excluded, what user-facing outcomes the feature commits to. Concretely: every entry under `## Goals`, `## Non-goals`, `## User-facing changes`, and any equivalent product-shape claim in the brief body. For Class A contradictions, the authority order is:

```
brief.md > decisions.md > engineering-plan.md > chunk plan
```
...
| Chunk implementation (`/execute-plan`) and independent re-proof (`/implementation-verify`) | `sonnet` (intended) | Inline stations — the tier is set at invocation: factory runs `claude -p --model sonnet`; interactive runs ride the session model | The passed chunk plan is the contract that de-skills execution; verify re-runs gates and observes criteria literally |
