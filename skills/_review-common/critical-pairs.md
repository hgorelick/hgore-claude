...
**Not every active pair is defined here.** The brief-layer pairs (`P-BRIEF-*`) and spec-layer pairs (`P-SPEC-*`) are defined in `/brief-review-v2` and `/spec-review` respectively, and reach the persona through the `{active_critical_pair_subset}` slot in its prompt — which is authoritative. Apply the pairs named in your prompt whether or not they appear in this file.

---

## Universal (apply to all review types)

**P-CLASS-SCOPE — Class > line vs scope minimization.** Fix every instance in the *enumerated universe* — where the universe is the seed finding plus every sibling the same-round **class sweep** (`~/.claude/skills/_review-common/class-sweep.md`) surfaces for that class. Getting the universe right the first time no longer rests on the persona guessing every location: the dedicated sweep stage walks the peer-set exhaustively before fixes are applied, so widening happens *within the round*, deterministically, not by a persona over-reaching. The guardrail is unchanged in the other direction — a sweep widens only to genuine siblings of the *same* invariant property, each carrying its own verbatim evidence; it does NOT expand to "every defect that looks similar," and a candidate instance without its own evidence is not an instance. A different defect class the sweep notices in passing is a new persona finding, not a sibling. There are still no Round-2 widens: the sweep is what makes the universe right the first time.
